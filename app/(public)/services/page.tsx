...
const INCLUDES: Record<string, string[]> = {
  ...
  OFFICE: ["Vacuuming & mopping","Desk & surface wipe-down","Washroom sanitize","Kitchen area clean","Trash removal","After-hours scheduling"],
  ...
  AIRBNB: ["Full clean & sanitize","Linen swap & bed-making","Bathroom restock","Kitchen reset","Trash removal","Guest-ready inspection"],
};
        ...
        <div className="container-pub space-y-16">
          {SERVICES.map((s, i) => (
            <div key={s.id} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${i % 2 === 1 ? "lg:flex-row-reverse" : ""}`} style={{ flexDirection: i % 2 === 1 ? "row-reverse" : "row" }}>
              {/* Image */}
              <div className={`relative rounded-3xl overflow-hidden shadow-card-hover ${i % 2 === 1 ? "lg:order-2" : ""}`} style={{ height: 360 }}>
